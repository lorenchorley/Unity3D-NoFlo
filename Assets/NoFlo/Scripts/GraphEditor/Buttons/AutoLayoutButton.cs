...
public class AutoLayoutButton : MonoBehaviour {
    ...
    Dictionary<Component, profile> profilesByNode;
    public void Layout() {
        if (Graph == null)
            return;

        profiles = new List<profile>();
        profilesByNode = new Dictionary<Component, profile>();
        
        foreach (Component c in Graph.NodesByName.Values) {
    ...
    private List<profile> ForwardConnectedNodes(Component n) {
    ...
    private List<profile> BackwardConnectedNodes(Component n) {
        List<profile> list = new List<profile>();
