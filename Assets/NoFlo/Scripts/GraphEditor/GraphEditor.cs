...
namespace NoFloEditor { 
    ...
    public class GraphEditor : MonoBehaviour {
        ...
        // Selection option
        [NonSerialized]
        public Visualisation SelectedVisualisation;
        ...
        // The panel to render the graph to
        public RectTransform GraphRenderingPanel;

        // The node info dialog reference
        ...
        // Dragging threshold
        public float RemoveEdgeDraggingThreshold = 20;

        // Dragging data
        private bool isDragging;
        private Vector3 StartingDragOffset;
        private Vector3 StartingPosition;
        private Action draggingAction;
        private Action endDraggingAction;
        ...
        void Start() {
            ...
            if (!isOpen && CurrentGraph != null && CurrentGraph.RunOnStart && !CurrentGraph.HasDoneFirstRun()) { 
                Open(CurrentGraph);
        ...
        public void Open(Graph Graph) {
            ...
            if (isOpen)
                throw new Exception("Cannot open editor when already open");
            ...
            Graph.PrimaryExecutor.Events.OnStart.AddListener(HandleStartExecution);
            Graph.PrimaryExecutor.Events.OnStop.AddListener(HandleEndExecution);
            Graph.DebugExecutor.Events.OnStart.AddListener(HandleStartExecution);
            Graph.DebugExecutor.Events.OnStop.AddListener(HandleEndExecution);
        ...
        public void Close() {
            if (!isOpen)
                throw new Exception("Cannot close editor when already closed");
        ...
        private void EdgeDraggingAction() {
            ...
            if (mag > RemoveEdgeDraggingThreshold) {
                v.gameObject.SetActive(false);
            } else {
        ...
        private void StartDragging() {
            isDragging = true;
            EventOptions.StartedDragging.Invoke();
        }

        private void EndDragging() {
            endDraggingAction.Invoke();
            isDragging = false;
            EventOptions.EndedDragging.Invoke();
        }
